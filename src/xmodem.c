 ...
 *          When we receive a header from UART, it decides what action
 *          it shall take.
 * @param   void
 * @return  void
 */
	...
	while (X_OK == status) {
		...
		case X_STX:
			...
			if (X_OK == packet_status) {
				...
				/*
				 * If the error was flash related, then
				 * immediately set the error counter to max
				 * (graceful abort).
				 */
				...
				/*
				 * Error while processing the packet, either
				 * send a NAK or do graceful abort.
				 */
				status = xmodem_error_handler(&error_number,
							      X_MAX_ERRORS);
			}
		...
		case X_EOT:
			/* End of Transmission. */
			/*
			 * ACK, feedback to user (as a text), then jump to
			 * user application.
			 */
			(void)uart_transmit_ch(X_ACK);
...
static xmodem_status xmodem_handle_packet(uint8_t header)
{
	...
	/* We calculate it too. */
	uint16_t crc_calculated = xmodem_calc_crc(&received_packet_data[0u],
						  size);
	...
	if (X_OK == status) {
		...
		if (255u != (received_packet_number[X_PACKET_NUMBER_INDEX] +
			...
			/*
			 * The sum of the packet number and packet number
			 * complement aren't 255. The sum always has to
			 * be 255.
			 */
			status |= X_ERROR_NUMBER;
		}
	...
	if ((X_OK == status) &&
	    (FLASH_OK != flash_write(xmodem_actual_flash_address,
	...
	/*
	 * Raise the packet number and the address counters
 ...
 *          Raises the error counter, then if the number of the errors
 *          reached critical, do a graceful abort, otherwise send a NAK.
